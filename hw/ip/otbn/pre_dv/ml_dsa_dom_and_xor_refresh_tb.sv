module ml_dsa_dom_and_xor_refresh_tb ();
    ...
    // Instantiate the DUT.
    ml_dsa_dom_and_xor_refresh dut (
        .clk_i(clk),
        .rst_ni(rst_n),
    ...
    initial begin: golden_block
        ...
        while (1) begin
            @(posedge clk);
            #(ACQ_DELAY);
            y0p = (stim.a0 & stim.b0) ^ stim.c0 ^ stim.r1;
            y1p = (stim.a0 & stim.b1) ^ stim.r0;
            y2p = (stim.a1 & stim.b0) ^ stim.r0;
            y3p = (stim.a1 & stim.b1) ^ stim.c1 ^ stim.r1;
            gold_val.y0 = y0p ^ y1p;
            gold_val.y1 = y2p ^ y3p;
