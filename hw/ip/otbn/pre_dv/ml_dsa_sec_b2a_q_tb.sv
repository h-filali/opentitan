module ml_dsa_sec_b2a_q_tb ();
    ...
    typedef struct packed {
        logic [SHARE_WIDTH-1:0] share0, share1;
    } masked_coeff_t;
    ...
    initial begin: application_block
        ...
        while (n_stims < TOT_STIMS) begin
            @(posedge clk);
            #(APPL_DELAY);
            inp_valid = 1'($urandom_range(1));
            if (inp_valid) begin
                gamma = SHARE_WIDTH'($urandom());
                rand_val = RAND_WIDTH'($urandom());
                mask = SHARE_WIDTH'($urandom());
                secret = SHARE_WIDTH'($urandom_range(MODULUS-1));
                stim.share0 = secret ^ mask;
                stim.share1 = mask;
                #(ACQ_DELAY-APPL_DELAY);
                n_stims = n_stims + 1;
                // Wait is always ready.
                // Uncomment in case backpressure is implemented.
                // wait (inp_ready);
            end
        end
        @(posedge clk);
            ...
            if (acq_resp_queue.size() > 0 && exp_resp_queue.size() > 0) begin
                ...
                if (acq_resp_unmasked !== exp_resp) begin
                    ...
                    $display("Mismatch occurred at %d: acquired (%2x + %2x) mod q = %2x, expected %2x",
                        $time, acq_resp.share0, acq_resp.share1,
